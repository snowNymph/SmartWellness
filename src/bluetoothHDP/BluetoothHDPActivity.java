...
public class BluetoothHDPActivity extends Activity {
    ...
    private int HEALTH_PROFILE_SOURCE_DATA_TYPE;// = ToKeepVar.getInstance().getType();
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        HEALTH_PROFILE_SOURCE_DATA_TYPE = ToKeepVar.getInstance().getType();
        ...
        */
        mStatusMessage = (TextView) findViewById(R.id.bluetooth_status_message);

        mRes = getResources();
        mHealthServiceBound = false;
        // Initiates application registration through {@link BluetoothHDPService}.
        Button registerAppButton = (Button) findViewById(R.id.bluetooth_test_btn1);
        ...
        */
        ...
        Button connectButton = (Button) findViewById(R.id.bluetooth_test_btn2);
    ...
    public void choiceValue(){
    	if(ToKeepVar.getInstance().getType() == ConstantVar.BLOOD_PRESSURE){
    		int idx = 0;
    		StringBuilder strbuild = new StringBuilder();
    		dataVal = new String[(BluetoothHDPService.mParseData_blood_pressure.size()/2 )+1];
    		for(int i =0; i<BluetoothHDPService.mParseData_blood_pressure.size(); i+=2){
        		strbuild.append("sys: ");
        		strbuild.append(BluetoothHDPService.mParseData_blood_pressure.get(i));
        		strbuild.append(" dia: ");
        		strbuild.append(BluetoothHDPService.mParseData_blood_pressure.get(i+1));

        		dataVal[idx] = strbuild.toString();
        		strbuild.delete(0, strbuild.length());
        		idx++;
    		}
    	}
    	else if(ToKeepVar.getInstance().getType() == ConstantVar.BODY_WEIGHT){
    		dataVal = new String[BluetoothHDPService.mParseData_weight.size()];
    		dataVal = (String[]) BluetoothHDPService.mParseData_weight.toArray();
    	}
    	else
    		return;
    	
    	SelectMeasureDialogFragment measureDialog = SelectMeasureDialogFragment.newInstance(dataVal, 0);
    	//SelectMeasureDialogFragment measureDialog = SelectMeasureDialogFragment.newInstance(dataVal, dataVal.length);
    	measureDialog.show(getFragmentManager(), "measureDialog");
    }
    ...
    private void fillTextView(){
    	SharedPreferences pref = getSharedPreferences("bluetooth_measure_data", MODE_PRIVATE);
	    SharedPreferences.Editor edit = pref.edit();

	    edit.putString("data", mData);
	    edit.commit();
	    finish();
	    finish();
    }
    
    private void setMeasure(int position){
        mData = this.dataVal[position];
        mDataIndex = position;
    }
    ...
    public static class SelectMeasureDialogFragment extends DialogFragment {
    	
    	public static SelectMeasureDialogFragment newInstance(String[] data, int position){
    		SelectMeasureDialogFragment frag = new SelectMeasureDialogFragment();
    		Bundle args = new Bundle();
    		args.putStringArray("measure", data);
    		args.putInt("position", position);
    		frag.setArguments(args);
    	...
    	@Override
    	public Dialog onCreateDialog(Bundle savedInstanceState){
    		String[] measureData = getArguments().getStringArray("measure");
    		int position = getArguments().getInt("position", -1);
    		if(position == -1) position = 0;
    		
			return new AlertDialog.Builder(getActivity())
					.setTitle("select measure data")
					.setPositiveButton(R.string.ok, new DialogInterface.OnClickListener() {
							@Override
							public void onClick(DialogInterface dialog, int which) {
								((BluetoothHDPActivity)getActivity()).fillTextView();
							}
						})
					.setSingleChoiceItems(measureData, position, new DialogInterface.OnClickListener() {
							@Override
							public void onClick(DialogInterface dialog, int which) {
								((BluetoothHDPActivity)getActivity()).setMeasure(which);
							}
						}
					)
					.create();
